...
function timeSpanToString(startDate, endDate) {
  ...
  const timeSpanDate = new Date(timeSpanMs);
  return timeSpanDate.toISOString().split('T')[1].slice(0, -1);
}
...
function angleBetweenClockHands(date) {
  const hours = new Date(new Date(date).setMinutes(date.getMinutes() + date.getTimezoneOffset())).toLocaleTimeString('en-US').split(':')[0] % 12;
  const minutes = date.getUTCMinutes();
  const hourAngle = 0.5 * (60 * hours + minutes);
  const minuteAngle = 6 * minutes;
  let angleBetween = Math.abs(minuteAngle - hourAngle);
