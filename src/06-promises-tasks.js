...
function chainPromises(array, action) {
  return new Promise((resolve) => {
    let value;
    let i = -1;
    async function recursivePromiseResolver() {
      if (array[i] !== undefined) {
        const result = await array[i].catch(() => {});
        if (value === undefined) {
          value = result;
        } else if (result !== undefined) {
          value = action(value, result);
        }
      }
      i += 1;
      if (i < array.length) { recursivePromiseResolver(); } else {
        resolve(value);
      }
    }
    recursivePromiseResolver();
  });
}
